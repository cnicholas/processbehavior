...
### Added
- **The design report states where sigma comes from, and how precise it is, at the
  observed series length.** One line under ``Structure``, e.g.
  ``Series length: T=4. Sigma for X/mR rests on 3 moving ranges; on a stable process,
  80% of such estimates fall between 0.43x and 1.68x the true sigma.`` With every cell
  replicated it reads ``Sigma rests on within-cell replication (25 degrees of freedom),
  not on the time sequence.`` — a single time period is a complete study. Partial
  replication states both facts. The interval comes from ``MR_SIGMA_INTERVAL_80``
  (``spc_constants``), the p10/p90 of MRbar/d2 by n = 3..30 generated by
  ``validation/short_series_bands.py`` (contributed by @rabujamra in #114 / #119); above
  n = 30 the n = 30 row bounds the statement. Exposed as ``study.series_length``
  (``SeriesLengthPrecision``, frozen) and ``study.series_length_description``. It is a
  fact beside the design state, not a judgment on it: no threshold, label, or warning,
  and the design state, recommended chart and analysis menu do not move with T
  (pinned by test). Closes the series-length half of #114.
