...
class LossResult:
    ...
    def plot(
        ...
        *,
        ...
        if structured and self.pdc_by_factor and len(self.pdc_by_factor) >= 2:
            ...
            pdc_int_pct = (self.pdc_factor_interaction / self.total * 100) if self.total > 0 else 0.0
    ...
    def as_dict(self, round_to: int | None = None) -> dict:
        ...
        if self.pdc_by_factor is not None:
            d['pdc_by_factor'] = {k: round(v, r) for k, v in self.pdc_by_factor.items()}
    ...
    def __repr__(self) -> str:
        ...
        if self.pdc_by_factor and len(self.pdc_by_factor) >= 2:
            ...
            pdc_items = sorted(self.pdc_by_factor.items(), key=lambda x: x[1], reverse=True)
...
def assess_loss(
    ...
    if rsg_vars and len(rsg_vars) >= 2:
        pdc_by_factor, pdc_factor_interaction = _compute_pdc_decomposition(ads.effects, rsg_vars, pdc)
