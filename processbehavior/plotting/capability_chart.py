...
def create_capability_chart(
    ...
    *,
    ...
    if view not in ("current", "potential"):
        raise ValueError(
            f"view must be 'current' or 'potential', got {view!r}"
        )

    if view == "potential" and cap.sigma_hat_r2 is None:
        raise ValidationError(
            f"Cannot plot potential capability: {cap.potential_unavailable_reason}"
        )
...
def _build_index_text(
    cap: CapabilityResult, show_potential: bool, view: str = "current",
) -> str:
...
def _add_spec_lines(
    fig: go.Figure, specs: SpecLimits,
    *, row: int | None = None, col: int | None = None,
    ...
    if row is not None and col is not None:
        ...
        if specs.lsl is not None:
            fig.add_shape(
                type="line", x0=specs.lsl, x1=specs.lsl, y0=0, y1=1,
        ...
        if specs.usl is not None:
            fig.add_shape(
                type="line", x0=specs.usl, x1=specs.usl, y0=0, y1=1,
                xref=xref, yref=f"{yref} domain",
                line=dict(color=_SPEC_LINE_COLOR, width=2, dash="solid"),
                layer="above",
            )
        if specs.target is not None:
            fig.add_shape(
                type="line", x0=specs.target, x1=specs.target, y0=0, y1=1,
                xref=xref, yref=f"{yref} domain",
...
def _add_npl_lines(
    ...
    *, view: str = "current",
    ...
    if is_subplot:
        ...
        fig.add_shape(
            type="line", x0=lnpl, x1=lnpl, y0=0, y1=1,
            xref=xref, yref=f"{yref} domain",
            line=dict(color=center_color, width=1.5, dash="dash"),
            layer="above",
        )
        fig.add_shape(
            type="line", x0=unpl, x1=unpl, y0=0, y1=1,
            xref=xref, yref=f"{yref} domain",
            line=dict(color=center_color, width=1.5, dash="dash"),
            layer="above",
        )
    ...
    *, row: int | None = None, col: int | None = None,
    ...
    if is_subplot:
        ...
        if specs.lsl is not None and specs.lsl > xminp:
            fig.add_shape(
                type="rect", x0=xminp, x1=specs.lsl, y0=0, y1=1,
                xref=xref, yref=f"{yref} domain",
        ...
        if specs.usl is not None and specs.usl < xmaxp:
            fig.add_shape(
                type="rect", x0=specs.usl, x1=xmaxp, y0=0, y1=1,
                xref=xref, yref=f"{yref} domain",
                fillcolor=_SPEC_LINE_COLOR,
                opacity=_SPEC_SHADE_OPACITY,
                line_width=0,
