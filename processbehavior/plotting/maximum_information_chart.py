...
def create_maximum_information_chart(
    ...
    *,
    ...
    if view not in ("combined", "xmr", "histogram"):
        raise ValueError(
            f"view must be 'combined', 'xmr', or 'histogram', got {view!r}"
        )
...
def _add_individuals_panel(
    ...
    if row is not None and col is not None:
        ...
        # Limit annotations
        r = result.round_to
        _add_limit_annotation(fig, f"UPL={round(stats['x_upl'], r)}", stats["x_upl"],
                              theme, row, col)
        _add_limit_annotation(fig, f"CL={round(stats['x_mean'], r)}", stats["x_mean"],
                              theme, row, col)
        _add_limit_annotation(fig, f"LPL={round(stats['x_lpl'], r)}", stats["x_lpl"],
                              theme, row, col)
        ...
        for text, yval in [
            ...
            fig.add_annotation(
                x=1.0, xref="x domain", y=yval, yref="y",
