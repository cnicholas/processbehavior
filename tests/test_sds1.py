...
def _harmonize_dtypes(
    df_left: pd.DataFrame, df_right: pd.DataFrame, cols: list[str]
...
def add_replicate_index(
    df: pd.DataFrame, k_col: str, t_col: str, order_cols=None
) -> pd.DataFrame:
...
def compare_frames_by_map(
    ...
    return pd.DataFrame(rows).sort_values(
        ['num_mismatch','max_abs_diff'], ascending=[False, False]
    )
...
def test_analysis_dataset_sds1(df_SDS1: pd.DataFrame):
    ...
    assert np.allclose(
        (df_calc['Ybar_k'] + df_calc['Ybar_t'] - df_calc['Ybar']) + df_calc['R3'],
        df_calc[y], atol=eps
    )
    assert np.allclose(
        (df_calc['Ybar'] + df_calc['Ybar_kt'] - df_calc['Ybar_t']) + df_calc['R4'],
        df_calc[y], atol=eps
    )
    assert np.allclose(
        (df_calc['Ybar'] + df_calc['Ybar_kt'] - df_calc['Ybar_k']) + df_calc['R5'],
        df_calc[y], atol=eps
    )
