...
class TestBug2_DuplicateByValues:
    ...
    def test_duplicate_by_with_multiple_factors(self):
        ...
        with warnings.catch_warnings(record=True):
            warnings.simplefilter('always')
            result = study.execute(chart='X', by=['factor 1', 'factor 2', 'factor 1'])

        # Should work — equivalent to by=['factor 1', 'factor 2']
        result_clean = study.execute(chart='X', by=['factor 1', 'factor 2'])
        pd.testing.assert_frame_equal(result.charts['X']['data'], result_clean.charts['X']['data'])
...
class TestBug4_NModeAverageStatistics:
    ...
    def test_xbar_n_mode_average_reports_average_n(self):
        ...
        expected_avg = 3.0
        assert stats['N'] == expected_avg, f'Expected N={expected_avg} (average), got N={stats["N"]}'
    ...
    def test_n_mode_actual_still_reports_max_n(self):
        ...
        assert (stats['N'] is None and stats.get('limits_vary') is True) or isinstance(stats['N'], (int, float))
...
class TestBug6_LaneBoundaryUnderscore:
    ...
    def test_lane_boundary_with_underscore_factor_levels(self):
        ...
        assert lane_boundaries is not None, 'Expected lane boundaries for multi-factor-level data with by=[]'
...
class TestBug10_InsufficientStrataMetadata:
    ...
    def test_insufficient_strata_metadata_reported(self):
        ...
        assert metadata['insufficient_strata'] is not None, "Expected insufficient_strata to list stratum 'C'"
