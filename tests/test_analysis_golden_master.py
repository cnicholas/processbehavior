...
IS_CI = any(
    os.environ.get(v, "").lower() in ("true", "1")
    for v in ("CI", "GITHUB_ACTIONS", "TRAVIS", "CIRCLECI", "GITLAB_CI")
)
...
def _save_snapshot(scenario_name: str, chart_key: str, data: pd.DataFrame,
...
def _assert_chart_matches_snapshot(scenario_name: str, chart_key: str,
                                   chart_result: dict):
    ...
    assert snapshot is not None, (
        f"No golden-master snapshot for {scenario_name}/{chart_key}. "
        f"Run with REGEN_GOLDEN_MASTERS=1 to generate."
    )
...
class TestShapeInvariants:
        ...
        for chart_key in ("XmR", "R"):
            ...
            if "beyond_limits" in data.columns:
                ...
                assert unique_vals <= {-1, 0, 1}, (
                    f"{chart_key} beyond_limits has invalid values: {unique_vals}"
                )
    ...
    def test_lpl_le_center_le_upl_scalar(self):
        ...
        if _limits_are_scalar(stats):
            assert stats["lpl"] <= stats["center"] <= stats["upl"], (
                ...
                f"center={stats['center']}, UPL={stats['upl']}"
            )
        ...
        for stratum, s in stats.items():
            if _limits_are_scalar(s):
                assert s["lpl"] <= s["center"] <= s["upl"], (
                    f"Limit ordering violated for {stratum}: "
                    f"LPL={s['lpl']}, center={s['center']}, UPL={s['upl']}"
                )
    ...
    def test_xbar_lpl_le_center_le_upl(self):
        ...
        if _limits_are_scalar(stats):
            assert stats["lpl"] <= stats["center"] <= stats["upl"], (
                f"Xbar limit ordering violated: LPL={stats['lpl']}, "
                f"center={stats['center']}, UPL={stats['upl']}"
            )
        ...
        if boundaries:
            ...
            if isinstance(boundaries, list):
                ...
                assert positions == sorted(positions), (
                    f"Lane boundary positions not monotonic: {positions}"
                )
