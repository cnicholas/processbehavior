...
class TestRunRulesUnexpectedErrorPropagates:
    ...
    def test_unexpected_error_propagates(self, theme, caplog):
        ...
        with pytest.raises(RuntimeError, match='boom'), caplog.at_level(logging.ERROR):
            add_run_rules_visualization(fig, data, stats, 'X', 'value', 'x', theme, result=mock_result)
    ...
    def test_known_skip_returns_fig(self, theme, caplog):
        ...
        with caplog.at_level(logging.INFO):
            returned = add_run_rules_visualization(fig, data, stats, 'X', 'value', 'x', theme, result=mock_result)
...
class TestSteppedLimitMissingColumnLogsDebug:
    ...
    def test_missing_column_logs_debug_and_returns_fig(self, theme, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            returned = add_stepped_limit_line(fig, data, 'x', 'nonexistent_col', 'red', 'dash', 1.5, 'UPL', theme)
...
class TestZoneBoundariesMissingLogsDebug:
    ...
    def test_varying_limits_logs_debug(self, theme, caplog):
        with caplog.at_level(logging.DEBUG):
            result = calculate_zone_boundaries({'center': 50.0, 'upl': None, 'lpl': 35.0, 'limits_vary': True}, theme)
...
class TestRuleNumsNoUnderscore:
    ...
    def test_rule_without_underscore(self, theme):
        ...
        data = pd.DataFrame({'value': [1.0, 2.0, 3.0], 'x': [0, 1, 2]}, index=[0, 1, 2])
        stats = {'center': 2.0, 'upl': 4.0, 'lpl': 0.0}
