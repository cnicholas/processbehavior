...
def test_residuals_match_bishop_reference(sds_num: int) -> None:
    ...
    study = ProcessBehavior(input_df).formulate(response='y', factors=['factor 1', 'factor 2'], time='time')
    ds = study.dataset
    ...
    assert len(merged) == len(ref), (
        f'SDS {sds_num}: row count mismatch after merge (got {len(merged)}, expected {len(ref)})'
    )
