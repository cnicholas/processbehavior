...
def test_s_chart_column_contract():
    ...
    assert len(set(s_values)) > 1, (
        f"'s' must vary across subgroups (values to plot). Got: {s_values}. This is a BUG if all values are identical."
    )
...
def test_x_chart_column_contract():
    ...
    spec = {'analysis_type': 'X', 'response_var': 'weight', 'rsg_vars': ['group'], 'time_var': 'time'}
